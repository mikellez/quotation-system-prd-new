        ...
        <div class="card-body p-0">
            ...
            $columns = [
                ...
                [
                    ...
                    //'template' => '{view} {update} {revision} {delete} {cancel} {done}',
                    'template' => '
                        <div class="btn-group" role="group">
                            ...
                            <div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
                                {view} {update} {revision} {delete} {cancel} {done}
                            </div>
                        </div>
                    ',
                    'buttons'=>[
                        'view'=> function($url,$model) {
                            return in_array($model->status, [$model::STATUS_APPROVE, $model::STATUS_DONE, $model::STATUS_CONFIRM])  ? Html::a('View PDF', '#', [
                                'title' => Yii::t('app', 'View Quotation'),
                                //'target' => '_blank',
                                'class'=>'dropdown-item',
                                'onclick'=>"window.open('".Yii::$app->getUrlManager()->getBaseUrl()."/quotation-group/document?id=".$model->id."', '_blank')"
                            ]) : Html::a('View Quotation', ['view', 'id'=>$model->id], [
                        ...
                        'update' => function($url,$model) { if($model->status == $model::STATUS_PENDING && Yii::$app->user->can('update-quotation-status')) {
                                return  Html::a('Approve Quotation', ['#'], [
                                    'value' => Yii::$app->urlManager->createUrl(['quotation-group/update-status', 'id'=>$model->id]),
                        ...
                        /*'view'=> function($url,$model) {
                            return $model->status == $model::STATUS_APPROVE || $model->status == $model::STATUS_DONE ? Html::a('<span class="fas fa-eye"></span>', ['quotation-group/document', 'id'=>$model->id], [
                                'title' => Yii::t('app', 'View'),
                        ...
                        'done' => function($url,$model) {
                            if($model->status == $model::STATUS_APPROVE) {
                                return  Html::a('<span class="fas fa-check-circle text-success"></span>', ['#'], [
                                    'value' => Yii::$app->urlManager->createUrl(['point-collection/create', 'id'=>$model->id]),
                                    'class' => 'showModalButton',
                                    'id' => 'BtnModalId',
                                    'data-toggle'=> 'modal',
                                    'data-target'=> '#modal',
                                    'datatitle'=> 'Convert Sales'
                                ]);
                            }
                            return $model->status == $model::STATUS_APPROVE ? Html::a('<span class="fas fa-check-circle text-success"></span>', ['update-done', 'id'=>$model->id], [
